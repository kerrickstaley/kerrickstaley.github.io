...
svg.rect(frame_width_mm + mm, frame_height_mm + mm).move(doc_margin_mm, doc_margin_mm).stroke({color: '#f00', width: 0.1}).fill({opacity: 0, color: '#000'});
...
let shelf_path_array = new SVG.PathArray(
    [
        ['M', frame_width_mm / 2 - hole_width / 2 + doc_margin_mm, hole_to_top_dist + hole_width / 2 + doc_margin_mm],
...
let cut_path_array = new SVG.PathArray(
    [['M', frame_width_mm / 2 + hole_width / 2 + doc_margin_mm, hole_to_top_dist + hole_height - hole_width / 2 + doc_margin_mm]]
    + shelf_interior_curve
    + [
