...
frame_elem.setAttribute('width', (frame_width_mm + 2 * doc_margin_mm)  + 'mm');
frame_elem.setAttribute('height', (frame_height_mm + 2 * doc_margin_mm) + 'mm')

let svg = SVG().addTo('#laser-frame');
svg.viewbox(0, 0, frame_width_mm + 2 * doc_margin_mm, frame_height_mm + 2 * doc_margin_mm);

let mm = 0; // "mm"

svg.rect(frame_width_mm + laser_kerf_mm + mm, frame_height_mm + laser_kerf_mm + mm).move(doc_margin_mm, doc_margin_mm).stroke({color: '#f00', width: 0.1}).fill({opacity: 0, color: '#000'});
...
let shelf_path_array = new SVG.PathArray(
    [
        ['M', frame_width_mm / 2 - hole_width / 2 + doc_margin_mm + laser_kerf_mm / 2, hole_to_top_dist + hole_width / 2 + doc_margin_mm + laser_kerf_mm / 2],
...
let cut_path_array = new SVG.PathArray(
    [['M', frame_width_mm / 2 + hole_width / 2 + doc_margin_mm + laser_kerf_mm / 2, hole_to_top_dist + hole_height - hole_width / 2 + doc_margin_mm + laser_kerf_mm / 2]]
    + shelf_interior_curve
    + [
